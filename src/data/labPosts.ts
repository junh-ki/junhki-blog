...
export const labPosts: LabPost[] = ([
  ...
  },
  {
    ...
    excerpt:
      'GET, POST, PUT, and DELETE cover the basics. But PATCH, HEAD, OPTIONS, and TRACE each solve a distinct problem the common four cannot. This post covers what each verb is designed for, where each one is misused, how safety and idempotency shape API design, and how to implement each one correctly in Spring Boot.',
    tags: ['HTTP', 'REST', 'API Design', 'Web', 'Networking', 'Spring Boot', 'Java'],
    ...
    content: {
      ...
      sections: [
        ...
        {
          heading: 'Spring Boot: Implementing the Less-Common Verbs',
          body: 'PATCH\n\nAnnotate the handler with @PatchMapping and decide on a body format before writing any code. For most APIs, JSON Merge Patch is the right default: the client sends a partial JSON object and you apply only the fields that are present.\n\nA practical approach is to accept the body as Map<String, Object>. Iterate the map and update only the keys that appear, then save. This avoids the nullability ambiguity of a dedicated DTO, where a null field could mean "client sent null" or "client did not include the field at all" -- the map makes the distinction explicit. If a key is absent, the field was not touched.\n\nFor strict media-type handling, use @PatchMapping(consumes = "application/merge-patch+json") and accept a JsonNode. Then call objectMapper.updateValue(existingEntity, patchNode). Jackson deserializes the patch onto the existing object, overwriting only the fields present in the patch and leaving everything else untouched. No null ambiguity, no manual field-by-field branching.\n\nFor JSON Patch (RFC 6902), add the com.github.java-json-tools:json-patch dependency. Accept an ArrayNode, call JsonPatch.apply(patchNode, objectMapper.valueToTree(entity)), and deserialize the result back into your entity type. More verbose, but precise: append-to-array and move operations that JSON Merge Patch cannot express become straightforward.\n\nHEAD\n\nSpring MVC handles HEAD automatically for every @GetMapping handler. When a HEAD request arrives, DispatcherServlet runs the full handler method but discards the response body before writing it to the socket. Every header set by the handler -- Content-Type, Content-Length, ETag, Last-Modified, custom headers -- is sent as normal.\n\nThis means a correct @GetMapping that sets response headers accurately gives you HEAD for free. The only case where an explicit @RequestMapping(method = RequestMethod.HEAD) is needed: when HEAD should behave differently from GET, for example returning a Content-Length that reflects the real resource size without loading the full body into memory first.\n\nTo handle conditional requests automatically (If-None-Match, If-Modified-Since producing 304 Not Modified), register ShallowEtagHeaderFilter as a FilterRegistrationBean. Spring computes the ETag from the response body hash and short-circuits repeated requests with a 304, dropping the body transfer entirely.\n\nOPTIONS\n\nSpring MVC populates the Allow response header automatically based on the @RequestMapping annotations on the matched controller. A controller with @GetMapping and @PostMapping returns Allow: GET, HEAD, POST, OPTIONS without any extra code. You do not need to write an OPTIONS handler for this.\n\nFor CORS, the recommended approach is a global configuration via WebMvcConfigurer.addCorsMappings. Define allowed origins, methods, and headers in one place rather than scattering @CrossOrigin across controllers. Spring handles the OPTIONS preflight exchange entirely: it intercepts the preflight, applies the CORS configuration, and returns the Access-Control-Allow-* headers without the request reaching any controller method.\n\nThe one case where an explicit @RequestMapping(method = RequestMethod.OPTIONS) handler is useful: returning a custom JSON body for API discovery, describing the endpoint schema or accepted fields. Use produces = MediaType.APPLICATION_JSON_VALUE to constrain it and return a ResponseEntity with your descriptor object.\n\nTRACE\n\nDo not implement TRACE. Spring Boot with embedded Tomcat disables it by default. To make this explicit rather than relying on a container default, add a deny rule in your Spring Security HttpSecurity configuration: .requestMatchers(r -> "TRACE".equalsIgnoreCase(r.getMethod())).denyAll(). This blocks TRACE at the security layer regardless of which embedded server is used and documents the intent clearly for anyone reading the configuration later.',
        },
        {
